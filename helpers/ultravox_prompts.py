"""Ultravox collection-agent prompts (from tata_tele_collection_Ultravox)."""

AGENT_NAME = "Arjun"

DEFAULT_CUSTOMER_NAME = "Sir/Madam"
DEFAULT_AMOUNT = "25,000"
DEFAULT_BILLING_PERIOD = "March 2026"
DEFAULT_SERVICE_NAME = (
    "Monthly Telecom Services (Voice, Internet & Business Connectivity)"
)
...
def build_greeting(
    ...
    *,
    ...
    greetings = {
        "English": (
            f"Hi, this is {agent_name} from Tata Tele services regarding a pending payment "
            f"for {service_name}. "
            "Would you like to continue in English or Hindi, Marathi, Gujarati, Bengali?"
        ),
        "Hindi": (
            f"नमस्ते, मैं {agent_name} बोल रहा हूँ Tata Tele services से, "
            f"आपके {service_name} के pending payment के बारे में। "
            "क्या आप हिंदी में बात करना चाहेंगे या English, Marathi, Gujarati, Bengali?"
        ),
        "Marathi": (
            f"नमस्कार, मी {agent_name} बोलतोय Tata Tele services कडून, "
            f"आपल्या {service_name} च्या pending payment बद्दल। "
            "आपण मराठीत बोलू का, की English, Hindi, Gujarati, Bengali?"
        ),
        "Gujarati": (
            f"નમસ્તે, હું {agent_name} બોલું છું Tata Tele services તરફથી, "
            f"તમારા {service_name} ના pending payment વિશે। "
            "તમે ગુજરાતી માં વાત કરશો કે English, Hindi, Marathi, Bengali?"
        ),
        "Bengali": (
            f"নমস্কার, আমি {agent_name} বলছি Tata Tele services থেকে, "
            f"আপনার {service_name}-এর pending payment সম্পর্কে। "
            "আপনি কি বাংলায় কথা বলবেন, না English, Hindi, Marathi, Gujarati?"
        ),
    }
...
def build_system_prompt(
    *,
    agent_name: str = AGENT_NAME,
    customer_name: str = DEFAULT_CUSTOMER_NAME,
    amount: str = DEFAULT_AMOUNT,
    billing_period: str = DEFAULT_BILLING_PERIOD,
    service_name: str = DEFAULT_SERVICE_NAME,
    language: str = DEFAULT_LANGUAGE,
) -> str:
    greeting = build_greeting(language, agent_name=agent_name, service_name=service_name)
    return f"""
        ...
        OPENING (when you speak first on the call):
        {greeting}
        """.strip()
